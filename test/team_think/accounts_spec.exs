defmodule TeamThink.AccountsTest do
  ...
  describe "register_user/1" do
    ...
    test "validates email and password when given" do
      ...
      password_error =
        if String.length(user.password) < @min_password_length do
          "should be at least #{@min_password_length} character(s)"
        else
          "should be at most #{@max_password_length} character(s)"
        end
    ...
    test "validates maximum values for email and password for security" do
      ...
      assert "should be at most #{@max_email_length} character(s)" in errors_on(changeset).email

      assert "should be at most #{@max_password_length} character(s)" in errors_on(changeset).password
    end
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Accounts.register_user(%{email: user.email, password: user.password})
    ...
    test "registers users with a hashed password" do
      valid_user = build(:valid_user) |> Map.from_struct()
  ...
  describe "change_user_registration/2" do
    ...
    test "allows fields to be set" do
      %{email: email, password: password} = build(:valid_user)
    ...
    test "sends token through notification", %{user: user} do
      ...
      token =
        extract_user_token(fn url ->
          Accounts.deliver_update_email_instructions(user, new_email, url)
        end)
      ...
      assert user_token.context == "change:" <> new_email
    end
  end
