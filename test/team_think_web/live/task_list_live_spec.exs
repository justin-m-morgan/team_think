defmodule TeamThinkWeb.TaskListLiveTest do
  ...
  describe "Show" do
    ...
    test "should render the task_list's name and description", %{conn: conn, project: project, task_list: task_list}  do
      {:ok, show_live, _html} = live(conn, Routes.task_list_show_path(conn, :show, project, task_list))
    ...
    test "should be able to open a modal to edit the task_list details", %{conn: conn, project: project, task_list: task_list} do
      {:ok, show_live, _html} = live(conn, Routes.task_list_show_path(conn, :show, project, task_list))
    ...
    test "should be able to update the project details with data", %{conn: conn, project: project, task_list: task_list} do

      update_params = Factory.build(:task_list_params) |> Map.delete(:project_id)
      {:ok, show_live, _html} = live(conn, Routes.task_list_show_path(conn, :edit, project, task_list))
    ...
    test "should not be able to update the project details with invalid data", %{conn: conn, project: project, task_list: task_list} do
    ...
    test "should be able to navigate to the task_lists's tasks page", %{conn: conn, project: project, task_list: task_list} do
      {:ok, show_live, _html} = live(conn, Routes.task_list_show_path(conn, :show, project, task_list))
