defmodule TeamThinkWeb.TaskLiveTest do
  ...
  describe "Index LiveView" do
    ...
    test "should render a placeholder message when no task_lists assigned to project",
      ...
      {:ok, index_live, _html} =
        live(conn, Routes.task_index_path(conn, :index, project, task_list))
    ...
    test "should render an item for each task", %{
      conn: conn,
      project: project,
      task_list: task_list
    } do
    ...
    test "should be able to navigate to the task show page", %{
      conn: conn,
      project: project,
      task_list: task_list
    } do
    ...
    test "should be able to open the edit modal", %{
      conn: conn,
      project: project,
      task_list: task_list
    } do
    ...
    test "should be able to delete the task", %{
      conn: conn,
      project: project,
      task_list: task_list
    } do
    ...
    test "should be able to create a new task in a modal",
      ...
      {:ok, index_live, _html} =
        live(conn, Routes.task_index_path(conn, :new, project, task_list))
  ...
  describe "Show" do
    ...
    test "should render the task's name and description",
      ...
      {:ok, show_live, _html} =
        live(conn, Routes.task_show_path(conn, :show, project, task_list, task))
    ...
    test "should be able to update the task details with valid data",
         %{conn: conn, project: project, task_list: task_list, task: task} do
      update_params =
        Factory.build(:task_params) |> Map.delete(:task_list_id) |> Map.delete(:status)

      {:ok, show_live, _html} =
        live(conn, Routes.task_show_path(conn, :edit, project, task_list, task))
      ...
      assert show_live
             ...
             |> follow_redirect(
               conn,
               Routes.task_show_path(conn, :show, project, task_list, task)
             )
    ...
    test "should not be able to update the task details with invalid data",
      ...
      invalid_update_params =
        Factory.build(:invalid_task_params) |> Map.delete(:task_list_id) |> Map.delete(:status)
